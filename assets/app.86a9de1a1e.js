...
(function () {
  ...
  function renderClabsiPreventionHome(){return shell(`<button class="back-link" data-back="clabsiPathwaySelect">← Back</button><div class="card"><div class="section-label">CLABSI Prevention Pathway</div><h1 class="card-title">Central line prevention and maintenance observation</h1><div class="card-subtitle">Review the current central-line order and indication, validate continued necessity, and observe evidence-based maintenance practices.</div><div class="button-row"><button class="button primary" id="startClabsiPreventionPathway">Begin prevention review</button></div></div>`,`<div class="card"><div class="section-label">Data sources</div><span class="source-pill">EHR pull</span><span class="source-pill">Clinical judgment</span><span class="source-pill">Direct observation</span></div>`);}
  function renderClabsiPreventionObservation(){const d=clabsiPreventionStep();let body='';let can=false;
    if(d.step===1){body=`<div class="ehr-panel"><div class="section-label">Information from the EHR</div><div class="data-cell"><div class="data-label">Current central line order / indication</div><div class="data-value">${['Vasoactive medication infusion','Hemodynamic monitoring','Long-term IV therapy','Limited peripheral access'][Object.keys(window.PRAXSYS_PATIENTS).indexOf(state.patientId)%4]}</div></div></div><label class="ack-row"><input type="checkbox" id="clabsiPreventionAck" ${state.clabsiPreventionAck?'checked':''}><span>I acknowledge that I reviewed the current central line order and documented indication.</span></label>`;can=state.clabsiPreventionAck;}
